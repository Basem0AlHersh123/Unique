...
export default function StudentsPage() {
  ...
  const [resetPasswordTarget, setResetPasswordTarget] = useState<Student | null>(
    null
  );
  ...
  async function handleToggleTier(id: string, current: string) {
    ...
    try {
      ...
      showToast(
        newTier === "paid"
          ? t("admin.tier_changed_to_paid")
          : t("admin.tier_changed_to_free"),
        "success"
      );
      await fetchData();
    } catch (err) {
  ...
  const displayStudents = showDeactivated
    ? students
  ...
  return (
    <div className="space-y-6">
      ...
      <Card withGlass className="p-4 slide-up">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-3 rounded-xl border-2 transition-all duration-200 shrink-0 ${
              showDeactivated
      ...
      <div className="bg-surface/50 backdrop-blur-sm border border-border rounded-2xl overflow-hidden shadow-sm slide-up">
              ...
              {displayStudents.length === 0 ? (
                ...
                displayStudents.map((s, i) => {
                  ...
                  return (
                    <tr
                      key={s._id}
                      className={`hover:bg-surface-hover/50 transition-colors duration-200 slide-up ${
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          >
                            ...
                            {s.role === "admin"
                              ? t("admin.admin_label")
                              : s.role === "teacher"
                              ? t("admin.teacher_label")
                              : t("admin.student_label")}
                          </Badge>
                          {isDeactivated && (
                      ...
                      <td className="px-4 py-3">
                        <div className="flex gap-1">
                          {s.role === "student" && !isDeactivated && (
                            <button
                              onClick={() =>
                                handleUpgrade(s._id, s.name, s.email)
                              }
                          ...
                          <button
                            ...
                            title={
                              s.tier === "paid"
                                ? t("admin.change_to_free")
                                : t("admin.change_to_paid")
                            }
                          >
                            <Star
                              className={`w-4 h-4 ${
                                s.tier === "paid" ? "fill-primary" : ""
                              }`}
                            />
                          </button>
                          {isDeactivated ? (
      ...
      <ConfirmDialog
        ...
        message={t("admin.convert_student_confirm_msg").replace(
          "{name}",
      ...
      <ConfirmDialog
        ...
        message={
          lang === "ar"
            ? `هل أنت متأكد من تعليق حساب ${deactivateTarget?.name}؟`
            : `Are you sure you want to suspend ${deactivateTarget?.name}?`
        }
      ...
      <SuccessDialog
        ...
        message={t("admin.upgrade_success_msg").replace(
          "{name}",
          upgradeSuccess?.name ?? ""
        )}
      ...
      {resetPasswordTarget && (
        ...
        >
          <div
            className="bg-surface rounded-2xl border border-border p-6 w-full max-w-md mx-4 shadow-2xl scale-in"
          ...
          >
            ...
            <p className="text-sm text-text-secondary mb-4">
              {t("admin.reset_password_for")}{" "}
              <strong>{resetPasswordTarget.name}</strong>
            </p>
            <input
              type="password"
              value={newPassword}
            ...
            />
            {resetError && (
              <p className="text-sm text-danger mb-3 animate-slide-in-right">
                {resetError}
              </p>
            )}
            <div className="flex gap-3">
              <button
                onClick={() => setResetPasswordTarget(null)}
                className="flex-1 px-4 py-2.5 rounded-xl border-2 border-border text-text-secondary hover:bg-surface hover:scale-[1.02] active:scale-95 transition-all duration-200"
              >
                {t("common.cancel")}
              </button>
              <button
                ...
                className="flex-1 px-4 py-2.5 rounded-xl bg-gradient-to-r from-primary to-primary-dark text-white hover:shadow-lg hover:shadow-primary/20 disabled:opacity-50 hover:scale-[1.02] active:scale-95 transition-all duration-200"
              >
                {processing === resetPasswordTarget._id ? (
