...
export function LoadingScreen() {
  ...
  const messages = ["جاري التحميل...", "Loading...", "يُحضّر التجربة...", "Preparing your experience..."];
  ...
  return (
    <div className="fixed inset-0 bg-gradient-to-br from-background via-background to-primary/5 flex items-center justify-center overflow-hidden">
      {mounted &&
        particles.map((p) => (
          <div
            key={p.id}
      ...
      <div className="relative flex flex-col items-center gap-8 z-10">
        ...
        <div className="flex items-center gap-2">
          ...
          <span
            className="w-2.5 h-2.5 rounded-full bg-primary-dark animate-dot-bounce"
            style={{ animationDelay: "0.15s" }}
          />
          <span
            className="w-2.5 h-2.5 rounded-full bg-accent animate-dot-bounce"
            style={{ animationDelay: "0.3s" }}
          />
          <span
            className="w-2.5 h-2.5 rounded-full bg-primary animate-dot-bounce"
            style={{ animationDelay: "0.45s" }}
          />
        </div>

        <p key={messageIndex} className="text-lg font-medium text-text-secondary animate-fade-in-up">
      ...
      <style jsx>{`
        ...
        .animate-logo-dance {
          animation: logo-dance 4s ease-in-out infinite;
        }
        .animate-logo-spin {
